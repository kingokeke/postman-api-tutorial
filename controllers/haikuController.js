...
module.exports = {
    ...
    create: function (req, res) {
        auth.profile(req, res, function (err, result, profile) {
    ...
    update: function (req, res) {
        auth.profile(req, res, function (err, result, profile) {
            ...
            var id = req.params.id;
            haikuModel.findOne({_id: id, author: profile.email}, function (err, haiku) {
                if (err) {
                    return res.status(500).json({
                        message: 'Error when getting haiku',
                        error: err
    ...
    remove: function (req, res) {
        auth.profile(req, res, function (err, result, profile) {
            ...
            haikuModel.findOne({_id: id, author: profile.email}, function (err, haiku) {
                ...
                haikuModel.remove({_id: id, author: profile.email}, function (err) {
                    if (err) {
                        return res.status(500).json({
